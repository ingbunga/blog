...
박스는 $x, y, w, h$를 예측하는데, $x, y$는 각 셀의 몇%에 객체의 중심이 있는지이고, $w, h$는 전체 이미지 대비 몇%의 너비와 높이를 가지고 있는지이다. confidence 의 정의는 다음과 같다. ([[IoU]])
$$
Pr(Object) ∗ IOU_{pred}^{truth}
$$
또한 각 cell은 다음과 같은 conditional class probabilities 를 $C$개 예측해야한다.
$$
Pr(Class_i|Object)
$$
이러한 예측은 결국 $S\times S \times (B ∗ 5 + C)$ 텐서로 인코딩 될 수 있다.
...
논문에서는 PascalVOC 에서 YOLO를 쓰기 위해 $7 \times 7$ 로 나누고, 각 셀당 $2$개의 박스를 예측하며, $20$개의 클래스로 분류한다. $(S=7, B=2, C=20)$ 
